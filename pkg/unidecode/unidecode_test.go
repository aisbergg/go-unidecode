...
type testUnidecodeFn func(s string, errors unidecode.ErrorHandling, replacement string) (string, error)

func TestUnidecode(t *testing.T) {
	testUnidecode(t, func(s string, errors unidecode.ErrorHandling, replacement string) (string, error) {
		return unidecode.Unidecode(s, errors, replacement)
	})
}

func TestUnidecodeBytes(t *testing.T) {
	testUnidecode(t, func(s string, errors unidecode.ErrorHandling, replacement string) (string, error) {
		b, err := unidecode.UnidecodeBytes([]byte(s), errors, replacement)
		return string(b), err
	})
}
...
func TestAppend(t *testing.T) {
	testUnidecode(t, func(s string, errors unidecode.ErrorHandling, replacement string) (string, error) {
		...
		buf, err := unidecode.Append(buf, s, errors, replacement)
...
func TestAppendBytes(t *testing.T) {
	testUnidecode(t, func(s string, errors unidecode.ErrorHandling, replacement string) (string, error) {
		buf := make([]byte, 0, len(s)*2)
		buf, err := unidecode.AppendBytes(buf, []byte(s), errors, replacement)
		return string(buf), err
	})
}
...
func TestWriterString(t *testing.T) {
	testUnidecode(t, func(s string, errors unidecode.ErrorHandling, replacement string) (string, error) {
		buf := strings.Builder{}
		w := unidecode.NewWriter(&buf, errors, replacement)
		_, err := w.WriteString(s)
		return buf.String(), err
	})

	// a writer that does not implement StringWriter
	testUnidecode(t, func(s string, errors unidecode.ErrorHandling, replacement string) (string, error) {
		w := &customWriter{}
		uw := unidecode.NewWriter(w, errors, replacement)
		_, err := uw.WriteString(s)
...
func testUnidecode(t *testing.T, fn testUnidecodeFn) {
	...
	t.Run("ASCII", func(t *testing.T) {
		for n := 0; n < unicode.MaxASCII; n++ {
			...
			actual, err := fn(expect, unidecode.Ignore, "")
			if err != nil {
	...
	t.Run("Various Unicodes, Ignore Errors", func(t *testing.T) {
		cases := []testCase{
			{"", ""},
			{"abc", "abc"},
			{"北京", "Bei Jing "},
	...
	t.Run("Various Unicodes, Break on Errors", func(t *testing.T) {
		cases := []struct {
			input       string
			expectError bool
		}{
			...
			{"北京", false},
			{"abc北京", false},
			{"ネオアームストロングサイクロンジェットアームストロング砲", true},
			{"\ua500", true},
			{"\u1eff", true},
			{"\U000fffff", true},
		}
		for _, tc := range cases {
			_, err := fn(tc.input, unidecode.Strict, "")
			if tc.expectError {
				if err == nil {
				...
				if err != nil {
					t.Errorf("Input: %s, Expected no error, Got: %s", tc.input, err)
				}
			}
		}
	})

	t.Run("Various Unicodes, Replace on Errors", func(t *testing.T) {
		cases := []testCase{
			{"", ""},
			{"abc", "abc"},
			{"北京", "Bei Jing "},
	...
	t.Run("Various Unicodes, Preserve on Errors", func(t *testing.T) {
		cases := []testCase{
			{"", ""},
			{"abc", "abc"},
			{"北京", "Bei Jing "},
			{"abc北京", "abcBei Jing "},
			{"ネオアームストロングサイクロンジェットアームストロング砲", "neoaーmusutorongusaikuronzietsutoaーmusutoronguPao "},
			{"\ua500", "\ua500"},
			{"\u1eff", "\u1eff"},
			{"\U000fffff", "\U000fffff"},
		}
		for _, tc := range cases {
			actual, err := fn(tc.input, unidecode.Preserve, "")
			if err != nil {
				t.Errorf("Error: %s", err)
			}
			if actual != tc.expected {
	...
	t.Run("Converter 1", func(t *testing.T) {
		expected := "THE QUICK BROWN FOX JUMPS OVER THE LAZY DOG 1234567890"
		cases := []string{
		...
		for _, tc := range cases {
			actual, err := fn(tc, unidecode.Ignore, "")
			if err != nil {
				t.Errorf("Error: %s", err)
			}
			if actual != expected {
				t.Errorf("Input: %s, Expected: %s, Got: %s", tc, expected, actual)
			}
		}
	})

	t.Run("Converter 2", func(t *testing.T) {
		expected := "the quick brown fox jumps over the lazy dog 1234567890"
		cases := []string{
			"\uff54\uff48\uff45 \uff51\uff55\uff49\uff43\uff4b \uff42\uff52\uff4f\uff57\uff4e \uff46\uff4f\uff58 \uff4a\uff55\uff4d\uff50\uff53 \uff4f\uff56\uff45\uff52 \uff54\uff48\uff45 \uff4c\uff41\uff5a\uff59 \uff44\uff4f\uff47 \uff11\uff12\uff13\uff14\uff15\uff16\uff17\uff18\uff19\uff10",
